...
average_train=np.mean(reg_train,axis=0)
average_test=np.mean(reg_test,axis=0)
std_train=np.std(reg_train,axis=0)
std_test=np.std(reg_test,axis=0)

mahalanobis_train=np.max((reg_train-average_train.reshape(1,-1))**2/std_train.reshape(1,-1)**2,axis=1)
mahalanobis_test=np.max((reg_test-average_test.reshape(1,-1))**2/std_test.reshape(1,-1)**2,axis=1)


clf =  EllipticEnvelope(contamination=0.02)
clf.fit(mahalanobis_train.reshape(510,-1))
labels_all_train_pred=clf.predict(mahalanobis_train.reshape(510,-1))
print(labels_all_train_pred)
labels_all_test_pred=clf.predict(mahalanobis_test.reshape(110,-1))
print(labels_all_train_pred)
